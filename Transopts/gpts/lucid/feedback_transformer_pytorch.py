...
class RelativePositionBias(nn.Module):
    ...
    def _relative_position_bucket(relative_position, causal=True, num_buckets=32, max_distance=128):
        ...
        val_if_large = (
            max_exact + (torch.log(n.float() / max_exact) / math.log(max_distance / max_exact) * (num_buckets - max_exact)).long()
        )
...
class FeedbackTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        num_tokens = vocab_size if vocab_size > 0 else num_tokens
        self.seq_len = seq_len
        self.mem_len = mem_len
        if lin_proj:
            self.in_proj = nn.Linear(in_feat, dim, bias=False)
            self.pos = nn.Parameter(torch.randn(num_tokens, dim))
        ...
        for _ in range(depth):
            attn = Attention(dim=dim, heads=n_head, dim_head=dim_head, dropout=attn_dropout)
    ...
    def get_embd(self, x):
        if self.lin_proj:
            return self.in_proj(x) + self.pos[: x.shape[1]].unsqueeze(0)
