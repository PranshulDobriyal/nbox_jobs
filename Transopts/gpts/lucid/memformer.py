    ...
    def __init__(self, dim, depth, heads=8):
        ...
        for _ in range(depth):
            self.layers.append(
                nn.ModuleList(
                    [
                        Residual(PreNorm(dim, Attention(dim, heads=heads, rel_pos_emb=True, causal=True))),
    ...
    def __init__(
        ...
        *,
        ...
        self.encoder = TransformerWrapper(
            dim=dim,
            layer_blocks=Encoder(dim=dim, depth=depth, heads=heads),
            return_logits=False,
        ...
        self.final_ff = nn.Linear(dim, vocab_size)

    def get_initial_mem(self, batch_size):
        return repeat(self.memory_slots, "n d -> b n d", b=batch_size)

    def forward(self, src, mems=None, src_mask=None, tgt_mask=None):
